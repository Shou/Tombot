...
messageTopicDetection message = do
    logs <- queryLogs @(Text, Text, Text, Text, Maybe Text)

    let logMessages = map (view _2) logs
        document = Text.words $ messageContent message
        corpus = Vec.map (Vec.fromList . Text.words) logMessages
        tfIdts = map (\a -> tfIdt a document corpus) document
